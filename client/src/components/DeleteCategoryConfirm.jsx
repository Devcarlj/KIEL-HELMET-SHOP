...
const DeleteCategoryConfirm = ({ close, category, onConfirm, loading }) => {
        ...
        <section className='fixed inset-0 z-50 bg-neutral-800/60 backdrop-blur-sm flex items-center justify-center p-4'>
            <div className='bg-white max-w-md w-full rounded-2xl shadow-2xl overflow-hidden'>
                ...
                <div className='flex items-center justify-between p-4 border-b bg-red-50/50'>
                    <h1 className='font-bold text-slate-800 text-lg'>Delete Category</h1>
                    <button
                        onClick={close}
                ...
                <div className='p-6 grid gap-5'>
                    <p className='text-slate-600'>
                        Are you sure you want to delete <span className='font-semibold text-slate-800'>"{category.name}"</span>? This cannot be undone.
                    </p>
                    {category.image && (
                    ...
                    <div className='flex items-center gap-3'>
                        ...
                        <button
                            ...
                            className={`flex-1 py-3 text-white font-bold rounded-xl shadow-lg transition-all ${
                                loading ? 'bg-slate-400 cursor-not-allowed' : 'bg-red-500 hover:bg-red-600'
                            }`}
                        >
